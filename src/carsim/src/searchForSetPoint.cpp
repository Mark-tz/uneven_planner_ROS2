#include <iostream>
#include <rclcpp/rclcpp.hpp>

#include <pcl/io/pcd_io.h>
#include <pcl/point_types.h>
#include <pcl/kdtree/kdtree_flann.h>
#include <pcl_conversions/pcl_conversions.h>

#include <gazebo_msgs/srv/set_entity_state.hpp>
#include <geometry_msgs/msg/point.hpp>
#include <sensor_msgs/msg/point_cloud2.hpp>

using namespace std;

class SearchForSetPoint : public rclcpp::Node
{
private:
    string map_file;
    
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloudMap_3d;
    pcl::PointCloud<pcl::PointXY>::Ptr cloudMap_2d;
    
    pcl::KdTreeFLANN<pcl::PointXYZ> kd_tree_3d;
    pcl::KdTreeFLANN<pcl::PointXY> kd_tree_2d;
    
    geometry_msgs::msg::Point point;
    sensor_msgs::msg::PointCloud2 cloud_msg;
    
    rclcpp::Client<gazebo_msgs::srv::SetEntityState>::SharedPtr client;
    rclcpp::Subscription<geometry_msgs::msg::Point>::SharedPtr listener;
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr cloud_pub;
    rclcpp::TimerBase::SharedPtr vis_timer;

public:
    SearchForSetPoint() : Node("searchForSetPoint")
    {
        listener = this->create_subscription<geometry_msgs::msg::Point>(
            "/set_model_location", 1, 
            std::bind(&SearchForSetPoint::callback, this, std::placeholders::_1));
        
        cloud_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_map", 1);
        client = this->create_client<gazebo_msgs::srv::SetEntityState>("/gazebo/set_entity_state");
        
        cloudMap_2d.reset(new pcl::PointCloud<pcl::PointXY>());
        cloudMap_3d.reset(new pcl::PointCloud<pcl::PointXYZ>());
        
        this->declare_parameter<std::string>("map_file", "");
        this->get_parameter("map_file", map_file);
        
        pcl::PCDReader reader;
        reader.read<pcl::PointXYZ>(map_file, *cloudMap_3d);
        
        for (size_t i=0; i<cloudMap_3d->points.size(); i++)
        {
            pcl::PointXY p;
            p.x = cloudMap_3d->points[i].x;
            p.y = cloudMap_3d->points[i].y;
            cloudMap_2d->points.emplace_back(p);
        }
        
        cloudMap_3d->width = cloudMap_3d->points.size();
        cloudMap_3d->height = 1;
        cloudMap_3d->is_dense = true;
        cloudMap_3d->header.frame_id = "world";
        
        cloudMap_2d->width = cloudMap_2d->points.size();
        cloudMap_2d->height = 1;
        cloudMap_2d->is_dense = true;
        cloudMap_2d->header.frame_id = "world";
        
        pcl::toROSMsg(*cloudMap_3d, cloud_msg);
        
        kd_tree_3d.setInputCloud(cloudMap_3d);
        kd_tree_2d.setInputCloud(cloudMap_2d);
        
        vis_timer = this->create_wall_timer(
            std::chrono::seconds(1), 
            std::bind(&SearchForSetPoint::visCallback, this));
        
        RCLCPP_INFO(this->get_logger(), "gazebo simulator ready.");
    }

private:
    void callback(const geometry_msgs::msg::Point::SharedPtr point)
    {
        // 2d search
        vector<int> Idxs;
        vector<float> SquaredDists;
        pcl::PointXY pxy;
        pxy.x = point->x;
        pxy.y = point->y;
        
        kd_tree_2d.nearestKSearch(pxy, 1, Idxs, SquaredDists);
        
        // 3d find
        pcl::PointXYZ pxyz;
        pxyz = cloudMap_3d->points[Idxs[0]];
        
        auto request = std::make_shared<gazebo_msgs::srv::SetEntityState::Request>();
        request->state.name = "racebot";
        request->state.pose.position.x = pxyz.x;
        request->state.pose.position.y = pxyz.y;
        request->state.pose.position.z = pxyz.z + 0.1;
        request->state.pose.orientation.w = cos(point->z/2.0);
        request->state.pose.orientation.x = 0;
        request->state.pose.orientation.y = 0;
        request->state.pose.orientation.z = sin(point->z/2.0);
        request->state.twist.linear.x = 0.0;
        request->state.twist.linear.y = 0.0;
        request->state.twist.linear.z = 0.0;
        request->state.twist.angular.x = 0.0;
        request->state.twist.angular.y = 0.0;
        request->state.twist.angular.z = 0.0;
        request->state.reference_frame = "world";
        
        client->async_send_request(request);
    }
    
    void visCallback()
    {
        cloud_pub->publish(cloud_msg);
    }
};

int main(int argc, char *argv[])
{
    rclcpp::init(argc, argv);
    auto node = std::make_shared<SearchForSetPoint>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
