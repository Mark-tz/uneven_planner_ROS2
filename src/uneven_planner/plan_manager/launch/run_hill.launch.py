...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('map_name',   default_value='hill'),
        DeclareLaunchArgument('x_pos',      default_value='4.3'),
        DeclareLaunchArgument('y_pos',      default_value='-4.3'),
        DeclareLaunchArgument('z_pos',      default_value='1.0'),
        DeclareLaunchArgument('Y_pos',      default_value='1.57'),

        Node(
            package='plan_manager',
            executable='manager_node',
            name='manager_node',
            parameters=[ join(get_package_share_directory('plan_manager'), 'params', 'run_hill.yaml') ],
            remappings=[
                ('cmd',  cmd_topic),
                ('odom', odom_topic),
                ('traj', traj_topic),
